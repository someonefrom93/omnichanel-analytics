# Delta Spec: Backfill Loop (PR2b)

> Covers the 30-day backfill loop, pure date helpers, fail-soft iteration, and Click CLI flags.
> SCN-014 delta (Bronze S3 partition/filename contract) is in `openspec/specs/bronze-ingestion/spec.md`.
...
| 7 | One log row per day | `test_run.py::test_backfill_writes_one_log_row_per_day` |
| 8 | Fail-soft continues | `test_run.py::test_backfill_fail_soft_continues_on_error` |
| 9 | Without backfill unchanged | `test_run.py::test_backfill_false_no_iteration` |
| 10 | --backfill-days 0 rejected | `test_run.py::test_cli_backfill_days_zero_rejected` |
| 11 | --backfill-days 91 rejected | `test_run.py::test_cli_backfill_days_over_cap_rejected` |
